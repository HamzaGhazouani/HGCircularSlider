...
//  @nonobjc static let circleInitialAngle: CGFloat = -CGFloat(Double.pi / 2)
  ...
  /**
   ...
   - parameter circleMinValue:  the min value of circle(default: 0)
   - parameter circleMaxValue:  the max value of circle(default: pi * 2)
   - parameter aValue:          the original value
   - parameter oldIntreval:     the original interval

   - returns: the angle value
   */
  internal static func scaleToAngle(circleMinValue: CGFloat, circleMaxValue: CGFloat, value aValue: CGFloat, inInterval oldInterval: Interval) -> CGFloat {
  ...
  /**
   ...
   - returns: the value in the new interval
   */
  internal static func value(circleMinValue: CGFloat, circleMaxValue: CGFloat, inInterval newInterval: Interval, fromAngle angle: CGFloat) -> CGFloat {
   ...
   */
  private static  func angle(circleInitialAngle: CGFloat, circleMaxValue: CGFloat, from alpha: CGFloat, to beta: CGFloat) -> CGFloat {
    guard beta <= circleMaxValue else {
      return 0
    }

    let halfValue = circleMaxValue/2 + circleInitialAngle
